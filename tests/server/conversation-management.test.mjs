...
async function create(title = "Original", extra = {}) {
  return db.chat.create({ data: { title, ...extra } });
}
...
test("full conversation search finds titles and old Chinese or English message text without indexing media or tool internals", async () => {
  ...
  assert.equal((await routes.list.GET(request("?q=月光旅行", "GET", undefined, ""))).status, 401);
  await db.$disconnect();
...
test("search treats punctuation and FTS operators literally instead of widening the query", async () => {
  ...
  assert.equal(await db.chat.count({ where: {} }), 1);
});
...
test("SQL search indexes follow title edits, message edits, regeneration and cascading deletion", async () => {
  ...
  const snapshot = await getRegenerationSnapshot(chat.id, "user");
...
test("pinned ordering, archive state and normalized tags compose with stable scoped pagination", async () => {
  ...
  const chats = [];
  for (let i = 0; i < 7; i++) chats.push(await create(`Match ${i}`, { pinned: i < 3, lastMessageAt: date, id: `match-${i}` }));
  const archived = await create("Match archived", { pinned: true, archived: true });
  ...
  assert.equal((await routes.list.GET(request(`?cursor=${first.pageInfo.nextCursor}`, "GET", undefined, ""))).status, 401);
  await data(await update(archived.id, { archived: false }));
  assert.equal((await data(await routes.list.GET(request())))[0].id, archived.id);
});

test("conversation metadata and query validation reject malformed updates atomically", async () => {
  const chat = await create();
  for (const body of [{}, { title: "Changed", pinned: "true" }, { archived: 1 }, { tags: ["bad,tag"] }, { tags: ["bad，tag"] }, { tags: Array(9).fill("tag") }, { tags: ["x".repeat(33)] }, { }]) {
  ...
  for (const query of ["q=Original", "state=archived"]) {
    assert.equal((await routes.list.GET(request(`?${query}`, "GET", undefined, ""))).status, 401, query);
  }
});

test("bulk deletion requires explicit confirmation and retains shared private media", async () => {
  const first = await create("Delete one"), second = await create("Delete two"), keep = await create("Keep");
  const asset = await createMediaAsset({ mediaType: "image/png", kind: "attachment", bytes: Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a8XcAAAAASUVORK5CYII=", "base64") });
  ...
  assert.equal((await routes.bulk.POST(request("/bulk-delete", "POST", { ids: [first.id, "missing-chat"], confirm: true }))).status, 404);
  ...
  assert.ok((await getMediaAsset(asset.id)).lastUsedAt.getTime() > 0);
...
test("exports include complete ordered text and safe references without binary, paths or tool credentials", async () => {
  ...
  for (const marker of ["not-exported-binary", "not-exported-path", "not-exported-secret", "not-exported-owner"]) assert.equal(JSON.stringify(json).includes(marker), false);
  ...
  assert.equal((await routes.export.GET(request(`/${chat.id}/export?format=json`, "GET", undefined, ""), context(chat.id))).status, 401);
...
test("search maintenance handles stable IDs and deletes long histories without leaving indexed text", async () => {
  ...
  await db.message.createMany({ data: Array.from({ length: 1000 }, (_, i) => ({ id: `maintenance-enance-${i}`, chatId: chat.id, role: "user", content: `Maintenance content ${i}` })) });
