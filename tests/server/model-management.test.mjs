...
process.env.OPENROUTER_API_KEY="offline-placeholder";providerState.imageCalls.length=0;});
...
test("chat fallback is opt-in, records both attempts and only happens before output",async()=>{
  ...
  const settings=defaultModelPreferences();settings.chat.fallbackId="google/gemini-3-flash-preview";await saveModelPreferences(settings);
...
test("media fallback records actual selected models, respects defaults and leaves library regeneration on its original model",async t=>{
  const settings=defaultModelPreferences();settings.image.modelId="google/gemini-3.1-flash-image-preview";settings.image.fallbackId="google/gemini-2.5-flash-image";settings.rates[settings.image.fallbackId]={inputPerMillion:null,outputPerMillion:null,perRequest:0.03};await saveModelPreferences(settings);
...
test("model APIs enforce the local access credential, Origin, quotas and removed-model warnings without returning prompts or secrets",async()=>{
  ...
  const settings=defaultModelPreferences();settings.chat.modelId="removed/model";await db.workspacePreference.create({data:{id:"local", settings}});
