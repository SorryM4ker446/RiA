...
beforeEach(async () => {
  ...
  // Quotas belong to this instance, so a previous test must not consume them.
...
test("incremental updates retain unchanged chunk IDs, retries deduplicate, reindex repairs terms and deletion cascades", async () => {
  ...
  assert.deepEqual(await searchDocuments("蓝鲸备份"), []);
  ...
  assert.deepEqual(await searchDocuments("蓝鲸"), []);
});

test("document APIs require the local access credential for list, text, search, reindex and delete", async () => {
  ...
  assert.equal((await search.POST(request("POST", "", { query: "猎户座" }))).status, 401);
  // The workspace sees its own document; another installation cannot.
  assert.equal((await detail.GET(request("GET", cookie), context(document.id))).status, 200);
});
...
test("invalid content, size, compressed expansion and unexpected requests cannot replace a working index", async () => {
  ...
  // Document uploads are quota-limited per instance, so this test clears the bucket.
  globalThis.__privateAiRateLimitStore?.clear();
...
test("ingestion rate limits return Retry-After without modifying documents", async () => {
  ...
  assert.equal(await db.knowledgeDocument.count({ where: {} }), 1);
});
...
test("failed indexing and stale reindex keep previous document and its index atomic", async () => {
  ...
  await assert.rejects(indexDocument({ ...original, pages: [{ pageNumber: null, text: "a\n\n".repeat(300) }] }), error => error.code === "PAYLOAD_TOO_LARGE");
  await assert.rejects(indexDocument({ ...original, pages: original.pages }, { id: original.id, contentHash: "stale" }), error => error.code === "CONFLICT");
  assert.equal((await db.knowledgeDocument.findUnique({ where: { id: document.id } })).contentHash, original.contentHash);
  assert.ok((await searchDocuments("月球基地补给")).length);
});
...
test("a database failure during term insertion rolls back the text, chunks and previous search index", async t => {
  ...
  try {
    await assert.rejects(indexDocument({ ...original, pages: [{ pageNumber: null, text: "tripwire" }] }));
  ...
  assert.match((await searchDocuments("火星仓库"))[0].snippet, /每周四/);
});
...
test("document count cap is enforced transactionally while updates remain available", async () => {
  await db.knowledgeDocument.createMany({ data: Array.from({ length: 99 }, (_, index) => ({ filename: `quota-${index}.txt`, format: "txt", byteSize: 4, contentHash: "old", pages: [{ pageNumber: null, text: "seed" }], characterCount: 4, indexVersion: 1 })) });
  const settled = await Promise.allSettled(["last-a.txt", "last-b.txt"].map(filename => indexDocument({ filename, format: "txt", byteSize: 4, pages: [{ pageNumber: null, text: "last" }] })));
  assert.equal(settled.filter(result => result.status === "fulfilled").length, 1);
  assert.equal(settled.find(result => result.status === "rejected").reason.code, "CONFLICT");
  assert.equal(await db.knowledgeDocument.count({ where: {} }), 100);
  ...
  assert.equal((await upload("quota-1.txt", "updated in place too")).status, 200);
});
