...
async function getScopedMessage(conversationId: string, messageId: string) {
...
async function GETHandler(req: NextRequest, context: Params) {
  try {
    await requireLocalWorkspace(req);const { id: conversationId, messageId } = await context.params;

    const message = await getScopedMessage(conversationId, messageId);
    if (!message) {
      throw new ApiError({ code: "NOT_FOUND", message: "Message not found" });
    }

    return Response.json({ data: await migrateMessageMedia(message) });
  } catch (error) {
...
async function PATCHHandler(req: NextRequest, context: Params) {
  try {
    ...
    const existing = await getScopedMessage(conversationId, messageId);
    if (!existing) {
      throw new ApiError({ code: "NOT_FOUND", message: "Message not found" });
    }
    ...
    const updated = await db.$transaction(async (tx) => {
      ...
      if (prepared) await replaceMessageMedia(tx, message.id, prepared.assetIds);
      return message;
    });
