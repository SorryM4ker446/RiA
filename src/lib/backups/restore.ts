...
export async function restoreAccountBackup(id: string) {
  ...
  try {
    ...
    await db.$transaction(async tx => {
      ...
      for (let i = 0; i < manifest.chats.length; i += 250) await tx.chat.createMany({ data: manifest.chats.slice(i, i + 250).map(({ messages: _messages, tags: _tags, ...chat }) => ({ ...chat, id: mapped(chat.id) })) });
